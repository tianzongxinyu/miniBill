...
- **セルフホスト・マルチユーザー**：データはローカル SQLite に保存、ユーザーごとに独立した帳簿；Docker デプロイと FygoOS インストールパッケージに対応
...
軽帳簿は FygoOS ネイティブの `.fpk` インストールパッケージとしてパッケージ化でき、Docker なしで FygoOS 上で直接プロセスを実行できます。
...
1. FygoOS にログイン → **アプリセンター** → **手動インストール**
